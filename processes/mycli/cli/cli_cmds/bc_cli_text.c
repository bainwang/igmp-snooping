...
const char *bc_cli_menu[CMD_MID_MAXNUM][CMD_ML_MAXNUM] = 
{
	...
	{"数量", "num"}, //CMD_MID_NUM
	{"文件名称", "file name"}, //CMD_MID_FILE_NAME 
	{"文件大小", "file size"}, //CMD_MID_FILE_SIZE
	{"创建时间", "creation date"}, //CMD_MID_FILE_CREATE_DATA
	...
	{"接收速率", "Receive Rate"}, //CMD_MID_INGRESS_RATE
	{"发送速率", "Transmit Rate"}, //CMD_MID_EGRESS_RATE
	{"位/秒", "bps"}, //CMD_MID_BPS
	{"千位/秒", "kbps"}, //CMD_MID_KBPS	
	{"帧数/秒", "Frames/s"}, //CMD_MID_FRAMES_PS
...
//vlan
	...
	{"配置失败，因为表项已经配满!", "configure failed, because entry is full !\r\n"}, //CMD_MID_VLAN_FULL_NOTE
//igmp
...
//onu
	...
	{"黑名单", "blacklist"}, //CMD_MID_ONU_BLACK_LIST
	{"白名单", "whitelist"}, //CMD_MID_ONU_WHITE_LIST
	...
	{"授权模式", "authorization mode"}, //CMD_MID_ONU_AUTH
	{"loid", "loid"}, //CMD_MID_ONU_LOID_LIST
	{"loid+密码", "loid+password"}, //CMD_MID_ONU_LOID_PWD_LIST
	{"白名单+loid", "whitelist+loid"}, //CMD_MID_ONU_MIX_WHITE_LOID_LIST
	{"白名单+loid-密码", "whitelist+loid-pwd"}, //CMD_MID_ONU_MIX_WHITE_LOID_PWD_LIST
	{"逻辑ONU标识", "logical ONU identifier"},//CMD_MID_ONU_AUTH_LOID
	{"密码", "password"}, //CMD_MID_ONU_AUTH_PASSWORD
...
//uni
	...
	{"MAC 永不老化。", "MAC non-aging."}, //CMD_MID_NON_AGING
	{"MAC 老化时间为 %d 秒。", "aging time is %d second."}, //CMD_MID_AGING_TIME_IS
...
//statistics
	...
	{"长度为(1519~2047)字节包数", "packets(1519~2047)B"}, //CMD_MID_STATS_PKTS_1519_2047
...
//stp
	...
	{"网桥", "Bridge"}, //CMD_MID_STP_BRIDGE
	{"STP使能", "STP Enable"}, //CMD_MID_STP_ENABLE
	{"协议版本", "Protocol Version"}, //CMD_MID_STP_VER
	{"SSTP", "SSTP"}, //CMD_MID_SSTP
	{"RSTP", "RSTP"}, //CMD_MID_RSTP
	{"MSTP", "MSTP"}, //CMD_MID_MSTP
	
	{"拓扑变换检测", "Topology Change Detected"}, //CMD_MID_STP_TOPO_CHG_DET,
	{"拓扑变换", "Topology Change"}, //CMD_MID_STP_TOPO_CHG,

	{"优先级", "Priority"}, //CMD_MID_STP_PRI
	{"最新拓扑变更时间", "Last Topology Change"}, //CMD_MID_STP_CHG_TIME
	{"拓扑变更次数", "Topology Changes"}, //CMD_MID_STP_CHG_COUNT
	{"根路径开销", "Root Path Cost"}, //CMD_MID_STP_ROOT_PATH_COST
	{"老化时间", "Max Age"}, //CMD_MID_STP_MAX_AGE
	{"BPDU发送间隔", "Hello Time"}, //CMD_MID_STP_HELLO_TIME
	{"占用时间", "Hold Time"}, //CMD_MID_STP_HOLD_TIME
	{"转换时延", "Forward Delay"}, //CMD_MID_STP_FWD_DELAY
	{"每秒最多发送BPDU个数", "Transmit Hold Count"}, //CMD_MID_STP_TXHOLD_CNT
	{"最大跳数", "Max Hops"}, //CMD_MID_STP_MAX_HOPS
	...
	{"路径开销算法", "Path Cost Method"}, //CMD_MID_STP_COST_METHOD
	{"短", "Short Method"}, //CMD_MID_STP_COST_METHOD_SHORT,
	{"长", "Long Method"}, //CMD_MID_STP_COST_METHOD_LONG,
	{"默认", "Default Method"}, //CMD_MID_STP_COST_METHOD_DEF,

	{"根端口索引", "Root Port Index"}, //CMD_MID_STP_ROOT_PORT_INDEX,

	{"根网桥ID", "Root ID"}, //CMD_MID_STP_ROOT_BR_ID,
	{"区域根网桥ID", "Region Root ID"}, //CMD_MID_STP_REG_ROOT_BR_ID,
	{"本网桥ID", "My ID"}, //CMD_MID_STP_BR_ID,

	{"Portfast BPDU过滤", "Portfast Bpdu-Filter"}, // CMD_MID_STP_PORTFAST_BPDU_FILTER
	{"Portfast BPDU保护", "Portfast Bpdu-Guard"}, // CMD_MID_STP_PORTFAST_BPDU_GUARD
	{"Portfast禁用超时", "Portfast Errdisable Timeout"}, // CMD_MID_STP_PORTFAST_ERRD_TO
	{"Portfast禁用超时间隔", "Portfast Errdisable Timeout Interval"}, // CMD_MID_STP_PORTFAST_ERRD_TO_ITV

	{"端口信息", "Port Information"}, // CMD_MID_STP_PORT_INFO
	{"端口名称", "Port Name"}, // CMD_MID_STP_PORT_NAME,
	{"端口索引", "Port Index"}, // CMD_MID_STP_PORT_INDEX,
	{"端口ID", "Port ID"}, // CMD_MID_STP_PORT_ID,
	{"端口角色", "Port Role"}, // CMD_MID_STP_PORT_ROLE,
	{"端口状态", "Port State"}, //CMD_MID_STP_PORT_STATE

	{"路径开销", "Path Cost"}, // CMD_MID_STP_PATH_COST,
	{"外部根路径开销", "External Root Path Cost"}, // CMD_MID_MSTP_EXTERNAL_ROOT_PATH_COST,
	{"内部根路径开销", "Internal Root Path Cost"}, // CMD_MID_MSTP_INTERNAL_ROOT_PATH_COST,

	{"端口树数", "Ref Count"}, // CMD_MID_MSTP_REF_COUNT
	{"指定端口ID", "Designated Port ID"}, // CMD_MID_STP_DESIGNATED_PORT_ID

	{"CIST根网桥ID", "CIST Root Bridge ID"}, // CMD_MID_MSTP_CIST_ROOT
	{"指定网桥ID", "Designated Bridge ID"}, // CMD_MID_MSTP_DESIGNATED_BRIDGE
	{"消息时间", "Message Age"}, // CMD_MID_MSTP_MESSAGE_AGE

	{"转发时延剩余计时", "Forward Timer Remain"}, // CMD_MID_MSTP_FORWARD_TIMER_REMAIN
	{"消息时间剩余计时", "Message Age Timer Remain"}, // CMD_MID_MSTP_MESSAGE_AGE_TIMER_REMAIN
	{"BPDU间隔剩余计时", "Hello Timer Remain"}, // CMD_MID_MSTP_HELLO_TIMER_REMAIN
	{"拓扑变化剩余计时", "Topology Change Timer Remain"}, // CMD_MID_MSTP_TOPOLOGY_CHANGE_TIMER_REMAIN

	{"状态跳转次数", "Forward Transitions"}, // CMD_MID_MSTP_FORWARD_TRANS
	{"Portfast配置", "Portfast Configed"}, // CMD_MID_MSTP_PORTFAST_CONFIG
	{"BPDU保护", "BPDU-Guard"}, // CMD_MID_MSTP_BPDU_GUARD
	{"当前BPDU保护", "Current BPDU-Guard"}, // CMD_MID_MSTP_CURRENT_BPDU_GUARD
	{"BPDU过滤" ,"BPDU-Filter"}, // CMD_MID_MSTP_BPDU_FILTER
	{"当前BPDU过滤", "Current BPDU-Filter"}, // CMD_MID_MSTP_CURRENT_BPDU_FILTER
	{"根保护可配", "Root-Guard Configured"}, // CMD_MID_MSTP_ROOT_GUARD_CONFIG
	{"当前根保护", "Current Root-Guard"}, // CMD_MID_MSTP_CURRENT_ROOT_GUARD
#endif
...
//igmp configure
	...
	{"组播包的预览间隔时间", "preview interval time"}, //CMD_MID_IGMP_PREVIEW_INTERVAL_TIME
	...
	{"业务包列表", "package list"}, //CMD_MID_PKG_LIST
