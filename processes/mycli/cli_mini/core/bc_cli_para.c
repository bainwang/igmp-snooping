 ...
 * HISTORY:
 * -----------------------------------------------------------------------------
 * Version 1.0 -- 2009-Mar-19 by wangbin
 * [NEW FEATURE]
 * <DESCRIPTION> Create
 *============================================================================*/
	...
	IN I32_T flags)
{
	...
	while (pwords->index < max_word)
	{
		...
		for (i = 0; i < count; i++)
		{
			...
			{
				/* add for parse the parameter of */
				if (cur_posible_node[i]->type == CLI_TYPES_SENTENCE)
				{
					/* the parameter of sentence must be matched but not the most match
					 * and the index should jump to the end, not search more */
					CLI_TYPES_GetValue(cur_posible_node[i], pwords->word[pwords->index], result);
					pwords->index = max_word - 1;
				}
				else
				{
