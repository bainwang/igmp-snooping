...
typedef struct stag_vt_io_ctrl_t
{
    ...
    UI32_T                  ipAddress;
    UI8_T                   ip6Address[16];
    /* Added by wangbin, 2008.06.18
     *  to fix bug PI-0137 of DGS-3700-12(G) on Kingtype ChengDu TLD.
     *
...
I32_T CLI_API_Execute_OneUserCmd(IN I32_T session_id, IN I8_T *cmd_buf)
{
...
#if 0
    /* Create filter */
    for (filter = session->cmd; *filter != '\0'; filter++)
    {
        /* skip string */
        if (*filter == '"')
        {
        ...
        if (*filter == '|')
        {
            *filter = '\0';
            filter++;
            session->hfilter = CLI_FILTER_Create_Filterpara(filter);
            break;
        }
    }
#endif
    ...
    level = cli_acc_get_lvl(session_id);
    cli2_value = NULL;
    cmd_node = CLI_CORE_Parse_Cmdstr(view, cmd_buf, level, &cli2_value, &(session->err_pos));
    CLI_ACC_SetPageStopStatus(session_id, CLI_ACC_GetPageStopStatus(session_id)); /* reset pagestop */
    ...
    if ((cmd_node == NULL) || (cmd_node == CLI_AMBIGUOUS_NODE))
    {
        ...
        else
        {
            ...
            for (i = 0; i < (session->err_pos + noEditeLen); i++)
            {
    ...
    if (session->hfilter)
    {
        CLI_FILTER_Destroy_Filterpara(session->hfilter);
...
static void __cli2_api_cfg_file_progress_rate(IN I8_T *cmd_buf)
{
	...
	CLI_API_Printf(l_mdl_id, "\rOLT Configuration...................................%2u%%", percent);
...
/**************************************************************************
FUNCTION NAME:
    __cli2_api_get_current_usrname

DESCRIPTION:
    get current cli user's name
...
**************************************************************************/
static void __cli2_api_get_current_usrname(OUT I8_T *usr_name)
{
	I32_T					i_usr = 0;
	I32_T					usr_num = 0;
	I8_T					name[BC_USERMGMT_NAME_LEN] = {0};
	bc_usermgmt_info_t		usr_info[BC_USERMGMT_USER_NUM];

	if (NULL == usr_name)
	{
		CLI_DBG_CPL(l_mdl_id, "user name is null!\r\n");
		return;
	}

	memset(usr_info, 0x00, sizeof(bc_usermgmt_info_t) * BC_USERMGMT_USER_NUM);
#if 0
	if (BC_ERR_OK != bc_usermgmt_get_list(usr_info, &usr_num))
	{
		return;
	}
#endif
	for (i_usr = 0; i_usr < usr_num; i_usr++)
	{
		/* online cli user */
		if (BC_USERMGMT_STATE_LOGIN == usr_info[i_usr].state)
		{
			strncpy(name, usr_info[i_usr].name, 32);
			break;
		}
	}

	strncpy(usr_name, name, BC_USERMGMT_NAME_LEN);
	return;
}
#endif
...
I32_T cli_main_process(IN I32_T map_sessionID, IN BOOL_T pseudo_enable)
{
	...
	if (TRUE == pseudo_enable)
	{
		CLI_ACC_SetScriptMode(map_sessionID, TRUE);
		bc_cli_set_cmdpath(map_sessionID, CLI_MOD_GLBCFG_PATH, CLI_MOD_GLBCFG_PROM, NULL);
	}
	else
	{
		CLI_ACC_SetScriptMode(map_sessionID, FALSE);
	}
#endif
    ...
    {	
        ...
        case '\n': /* execute command */
            CLI_ACC_SetPageStopStatus(map_sessionID, TRUE);
            cmd_ret = CLI_API_Execute_OneUserCmd(map_sessionID, session->cmd);
            CLI_ACC_SetPageStopStatus(map_sessionID, FALSE);
            switch (cmd_ret)
            {
        ...
        case '?':
            ...
            session->helplist.local_help = cli_acc_get_lan(map_sessionID);
            if (CLI_CORE_Get_Help(&session->helplist, view, session->cmd, level) < 0)
            {
                /* get no editing length */
                noEditeLen = CLI_EDIT_Get_PromptLen(session->hedit);
                for (i = 0; i < (session->helplist.error_pos + noEditeLen); i++)
                {
                    CLI_API_Printf(map_sessionID, " ");
                }
...
#if (MARVELL_GALTIS_INCLUDED == 1)
    /*if galtis function is enable, CLI should not put anything to serial port*/
    if (!DRV_GALTIS_FlagGet())
    {
        for (i = 0; i < len; i++, str++)
        {
            STDIO_putchar((char) * str);
        }
    }
#else
    for (i = 0; i < len; i++, str++)
...
#if 0//(MARVELL_GALTIS_INCLUDED == 1)
    /*if galtis function is enable, CLI should not get anything from serial port
    * it will
    */
    if (!DRV_GALTIS_FlagGet())
    {
        return (int) STDIO_getcharT(timeout);
    }
    else
    {
        return UTL_COM_CONSOLE_TIMEOUT;/* CLI timeout*/
    }
#else
    //rx_int = (int) STDIO_getcharT(timeout);
	{  
	  	char 	rx_ch;  
...
#if (MARVELL_GALTIS_INCLUDED == 1)
    /*if galtis function is enable, this action is meanless*/
    if (!DRV_GALTIS_FlagGet())
    {
...
#if 0
static void __console_UART_Init(void)
{
    UI32_T  baudRate;
    BOOL_T  bPtptStatus = FALSE;

#if L3_INC_PTPT
    bPtptStatus = LA3_PTPT_Get_If_Status();
#endif /* L3_INC_PTPT */
    ...
    case TRUE:
    case FALSE:
        switch (SYS_CPU_Get_OutBand_BaudRate())
        {
        case BAUD_RATE_9600:
            baudRate = DEV_UART_BAUDRATE_9600;
            break;

        case BAUD_RATE_19200:
            baudRate = DEV_UART_BAUDRATE_19200;
            break;

        case BAUD_RATE_38400:
            baudRate = DEV_UART_BAUDRATE_38400;
            break;

        case BAUD_RATE_115200:
            baudRate = DEV_UART_BAUDRATE_115200;
            break;
        default:
            baudRate = DEV_UART_BAUDRATE_115200;
            break;
        ...
        /* end switch-case */
        /* set baudrate */
        DEV_UART_Set_Baudrate(PROJ_OS_DEFAULT_UART, baudRate);
        break;
...
static BOOL_T __cli2_io_check_loginbyloginlocal(IN const UI32_T sessionID, OUT bc_usermgmt_info_t *user_item)
{
    ...
    while (retry_times < BOOTUP_LOGIN_MAXRETRYTIME)
    {
        ...
        if (strlen(username_buf) < 32)
        {
			...
			if (BC_ERR_OK == bc_usermgmt_login(username_buf, password_buf, &level))
			{
        ...
        //CLI_ACC_GetLoginName(sessionID, login_name);
        CLI_ACC_SetLoginName(sessionID, user_item->name);
        //CLI_PORT_Log(sessionID, CLI_LOG_FAIL);
        //CLI_ACC_SetLoginName(sessionID, login_name);
...
static BOOL_T __cli2_io_check_loginbylinelogin(IN const UI32_T sessionID, IN UI32_T line)
{
    ...
    /*need add log, change the session login name to want set login name */
    CLI_ACC_GetLoginName(sessionID, login_name);
    CLI_ACC_SetLoginName(sessionID, PRIVMGMT_ANONYMOUS_USER);
    CLI_PORT_Log(sessionID, CLI_LOG_FAIL);
    CLI_ACC_SetLoginName(sessionID, login_name);
    ...
    )
{
    ...
    if (sessionID == VT_RS232_CHANNEL)
    {
        ...
        for (; ;)
        {
            rxChr = VT_Get_Char(sessionID, 100);
    ...
    if (ret)
    {
        ...
        CLI_ACC_SetLoginTime(sessionID, &login_time, ms);
        CLI_ACC_SetLoginName(sessionID, user_item.name);
        cli_acc_set_lvl(sessionID, (UI8_T) user_item.level);
    ...
    )
{
    ...
    sessionID = CLI_API_CreateSession(ptrIcb->sessionID, ptrIcb->flags, &terminal_io, CLI_VT100, &ip_info, NULL);
    if (sessionID < 0)
    {
        return;
    }

    cli_acc_set_lvl(sessionID, level);
    CLI_ACC_SetPageStopStatus(sessionID, FALSE);
    ...
    if (1)
    {
        ...
        cli_main_process(sessionID, FALSE);

        /* logout process */
        //__cli2_io_logout_process(sessionID, ret_flag);
    }

    CLI_API_DestroySession(sessionID);
