 ...
 * HISTORY:
 * -----------------------------------------------------------------------------
 * Version 1.0 -- 2009-May-31 by wangbin
 * [NEW FEATURE]
 * <DESCRIPTION> Create
 *============================================================================*/
...
void cli_pseudo_task (void *arg)
{
...
#if 1
	/* open default configure file and get configure command data */
	fp_cfg = bc_file_open("bc_boot_usr.cfg", BC_VFS_READ, BC_VFS_CFG_FILE_PATH, l_mdl_id);
	if (NULL == fp_cfg)
	{
		CLI_DBG_CPL(l_mdl_id, "open config file failed!\r\n");
		return;
	}

	p_cfg = bc_platform_mem_malloc(l_mdl_id, 1200*1024);
	if (NULL == p_cfg)
	{
	...
	bc_file_read(p_cfg, 1200*1024, 1, fp_cfg);
	bc_file_close(fp_cfg, l_mdl_id, "bc_boot_usr.cfg");
	...
	sessionID = CLI_API_CreateSession(VT_PSEUDO_CHANNEL, CLI_SESN_TYPE_PSEUDO, &tio, vio_type, &ip_info, NULL);
	if (sessionID < 0)
	{
		return;
	}

	cli_acc_set_lvl(sessionID, CLI_ACC_LVL_DEBUG);
