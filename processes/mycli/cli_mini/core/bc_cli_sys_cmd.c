...
**************************************************************************/
static I32_T __cli2_cmd_show_session
    (
    IN const UI32_T sid,
    IN cli_value_t 	*cli_para,
    IN void 		*user_data
...
{
    BOOL_T          bFirstHeader    = TRUE;
    cli_usr_info_t  userInfo;
    I8_T            leveString[20];
    I32_T           count;
	UI32_T			time;
    ...
    {
        count = 0;
        if (!bc_cli_get_usr_first_info(&userInfo))
        {
            CLI_API_Printf(sid, "\n\r No Session");
            return CLI_CMD_FAIL_PRT_NEWLINE;
        }
        ...
        {
            /* print header, ipv6 address string maximum length = 39. */
            CLI_API_Printf(sid, "\n\r ID  Live Time    From                                    Level User           ");
            CLI_API_Printf(sid, "\n\r --- ------------ --------------------------------------- ----- ---------------");

            bFirstHeader = FALSE;
        }

        if (CLI_PRT_Start_Paging(sid, 2) == CLI_PAGE_EXIT)
        {
            break;
        }
        ...
        do
        {
            /* just display the user login session */
            if (userInfo.sessionID < VT_PSEUDO_CHANNEL)
            {
                /* if want to call printf just one time, some bug happen */
                CLI_API_Printf(sid, "\n\r %-4d", userInfo.sessionID);
				bc_platform_time_sec_get(&time);
                CLI_API_Printf(sid, "%-13u", time - userInfo.loginTime);

                if (userInfo.ip6Flag != 0)
                {
                    CLI_API_Printf(sid, "%-40s", userInfo.ip6Address);
                }
                else if ((userInfo.ip6Flag == 0) && (userInfo.ipAddress != 0))
                {
                    CLI_API_Printf(sid, "%-40d", userInfo.ipAddress);
                }
                else
                {
                    CLI_API_Printf(sid, "%-40s", "Serial Port");
                }

                CLI_BNR_Trans_AccLevel_To_String(leveString, userInfo.level);
                CLI_API_Printf(sid, "%-6s", leveString);

                CLI_API_Printf(sid, "%-15s", userInfo.name);
                count++;
            }
        } while (bc_cli_get_usr_next_info(&userInfo));

        /* show total count */
        CLI_API_Printf(sid, "\n                                                            ");
        CLI_API_Printf(sid, "\n\rTotal Entries: %d", count);
    }
...
void bc_cli_sys_cmd_reg(void)
{
	...
	bc_cli_reg_cmd(
		CLI_MOD_GLOBAL_PATH
		"/command_history "		"#set command history#""#设置历史命令#"
	...
	bc_cli_reg_cmd(
		CLI_MOD_GLOBAL_PATH
		"/show "
		"command_history "		"#Display history command#""#显示历史命令#"
		,
		__cli2_cmd_show_hist,
		CLI_ACC_LVL_PRIVI,
		"Used to display command history.",
