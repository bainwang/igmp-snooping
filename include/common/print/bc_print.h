...
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
#define BC_COMPANY             		"BC"

#define BC_PT_PRJ_MDL_NUM_MAX		32
#define BC_PT_MDL_NAME_LEN_MAX		32

#define BC_PT_MDL_FILE_NUM_MAX		4
...
///自定义终端打印声明
int bc_pt_tty_printf(const char *format, ...);




#define	BC_WRITE_LOG_FUN(x1, x2, x3, x4, x5, fmt, ...)	\
	do\
	{\
		bc_log_wrt_sys_log(x1, x2, x3, x4, x5, fmt, ##__VA_ARGS__);\
	} while (0)
...
#define BC_PT_DBG_FUN(module_id, fmt, ...)		\
	...
	{\
		...
		/*	bc_pt_tty_printf(fmt, ##__VA_ARGS__);\*/	\
			BC_PT_FLUSH();\
		}\
	} while(0)
...
#define BC_PT_DBG_PRINT(module_id, fmt, ...) \
	do \
	{ \
		BC_PT_FUN(fmt, ##__VA_ARGS__);\
		BC_PT_FLUSH();\
...
#define	BC_PT_INIT(module_id, fmt, ...)		\
	...
	{\
		...
		BC_WRITE_LOG_FUN(BC_LOG_BIT_INIT, module_id, __FILE__, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__);\
	} while (0)
...
#define	BC_PT_EVENT(module_id, fmt, ...)	\
	...
	{\
		...
		BC_WRITE_LOG_FUN(BC_LOG_BIT_EVENT, module_id, __FILE__, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__);\
	} while (0)
...
#define	BC_PT_WARNING(module_id, fmt, ...)		\
	...
	{\
		...
		BC_WRITE_LOG_FUN(BC_LOG_BIT_WARNING, module_id, __FILE__, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__);\
	} while (0)
...
#define	BC_PT_ERROR(module_id, fmt, ...)		\
	...
	{\
		...
		BC_WRITE_LOG_FUN(BC_LOG_BIT_ERROR, module_id, __FILE__, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__);\
	} while (0)

/* move the command print to bc_cli.h */
#if 0	/*enable by bain.wang@outlook.com*/
...
/*************************************************
  Function: bc_pt_cfg_clr
  Description: 清除打印配置信息
  Input: 
  		void
  Output:
  		NULL
  Return:
  		0: 
  		~0: 
  Note: OLT和CLI进程清除调试配置信息调用
  History: 
*************************************************/
void bc_pt_cfg_clr(void);
...
/*************************************************
  Function: bc_pt_db_dump
  Description: dump 内存数据
  Input: 
  		void
  Output:
  		NULL
  ...
  History: 
*************************************************/
bc_err_e bc_pt_db_dump(bc_uint32 *pt_lvl, bc_pt_prj_flg_t *pt_prj);
